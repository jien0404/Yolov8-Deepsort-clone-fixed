...
class BasePredictor:
    ...
    def __call__(self, source=None, model=None):
        ...
        LOGGER.info(f"Total detect time   : {self.total_detect_time:.4f}s")
        LOGGER.info(f"Average detect/frame: {self.total_detect_time/self.frame_count:.4f}s")
        LOGGER.info(f"Total track time    : {self.total_track_time:.4f}s")
        LOGGER.info(f"Average track/frame : {self.total_track_time/self.frame_count:.4f}s")
        LOGGER.info(f"Total detect+track  : {(self.total_detect_time+self.total_track_time):.4f}s")
        LOGGER.info(f"FPS overall detect+track: {self.frame_count/(self.total_detect_time+self.total_track_time):.2f}")
